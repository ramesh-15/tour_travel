...
export default function UserPortal({ apiBaseUrl, session, onLogout }) {
  ...
  const [booking, setBooking] = useState({ tour_id: '', travel_date: '', travellers: 2, special_requests: '' })
  ...
  const loadDashboard = async () => {
    ...
    setLoading(true)
    try {
      const [accountResponse, bookingResponse, tourResponse] = await Promise.all([
        ...
        fetch(`${apiBaseUrl}/api/tours?page=1&page_size=100`),
      ])
      if (accountResponse.status === 401 || bookingResponse.status === 401) { onLogout(); return }
      const [accountBody, bookingBody, tourBody] = await Promise.all([accountResponse.json(), bookingResponse.json(), tourResponse.json()])
      ...
      if (tourResponse.ok) setTours(tourBody.items || [])
    } catch (error) {
  ...
  const submitBooking = async (event) => {
    event.preventDefault(); setSubmitting(true); setStatus('')
    try {
      const response = await fetch(`${apiBaseUrl}/api/bookings`, { method: 'POST', headers: { ...authHeaders(session.token), 'Content-Type': 'application/json' }, body: JSON.stringify({ ...booking, tour_id: Number(booking.tour_id), travellers: Number(booking.travellers) }) })
      const body = await response.json()
      if (!response.ok) throw new Error(body.detail || 'Unable to save your booking request.')
      setBookings(current => [body, ...current]); setBooking({ tour_id: '', travel_date: '', travellers: 2, special_requests: '' }); setActiveTab('bookings'); setStatus('Your booking request is saved. Complete the demo payment when you are ready.')
    } catch (error) { setStatus(error.message) } finally { setSubmitting(false) }
  }
